...
print_summary() {
    ...
    if [[ "$SETUP_MODE" == "daemon" ]]; then
        ...
        echo "    1. The daemon listens for Slack messages instantly (Socket Mode)"
        ...
        echo "    Start:    cd $SCRIPT_DIR && ./run.sh"
        echo "    Stop:     launchctl stop com.mission-control.daemon  (macOS)"
        echo "    Status:   cd $SCRIPT_DIR && ./status.sh"
        echo "    Logs:     ls $SCRIPT_DIR/logs/"
        echo "    Errors:   cat ~/.mission-control.err"
        echo "    Test:     cd $SCRIPT_DIR && ./run.sh --once"
        echo "    Re-setup: cd $SCRIPT_DIR && ./setup.sh"
    fi
    echo ""
